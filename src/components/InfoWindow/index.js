...
class RouteBoxInfoWindow extends Component {
  ...
  printWebsite(website) {
    return (
      <p>
        <a href={website}>{this.truncate(website.split('?')[0], 50, '...')}</a>
      </p>
    )
  }
  ...
  render() {
    ...
    return (
      ...
      >
        <div>
          <p><strong style={{ fontSize: '125%' }}>{marker.name}</strong></p>
          <p>{marker.details.formatted_address}</p>
          <div className="place-photo">
            {
              marker.details.photos && marker.details.photos.length &&
              this.printPhoto(marker.details.photos[0])
            }
          </div>
          <div className="place-details">
            ...
            { marker.details.website && this.printWebsite(marker.details.website) }
            {
              ...
              && this.printPhoneNumber(marker.details.formatted_phone_number)
            }
          </div>
        </div>
      </InfoWindow>
