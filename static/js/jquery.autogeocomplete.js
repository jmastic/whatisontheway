...
(function($) {
    ...
    $.fn.extend({
        ...
        autogeocomplete: function(options){
            ...
            this.autocomplete({
                ...
                open: function(event, ui){
                    $("#" + map_frame_id).css("display", "block");
                    $("#" + map_frame_id).css("z-index", "9999");
                    $("#" + map_window_id).css("z-index", "9999");
                    $(".ui-autocomplete").css("z-index", "9998");
                    
					if(this.id.toString().toLowerCase() == "end") {
						$("#"+map_frame_id).css("top", "170px");
					}
					else {
						$("#"+map_frame_id).css("top", "120px");
					}
                ...
                close: function(event, ui){
                    $("#"+map_frame_id).css("display", "none");
                },
                // update map rendering on mouseover / keyover
                focus: function(event, ui){
                    setMap(ui.item.latitude, ui.item.longitude);                
                },
				// When an address is selected from the drop down...
				select: function(event, ui) {
					if(!bothElementsSet()) {
						map_location = new google.maps.LatLng(ui.item.latitude, ui.item.longitude);
						// Set the particular marker at this location
						if(this.id.toString().toLowerCase() == "start")
							startmarker.setPosition(map_location);
						else if(this.id.toString().toLowerCase() == "end")
							endmarker.setPosition(map_location);
            ...
            function setMap(lat, lng){
                ...
                autocomplete_map.panTo(map_location);
            }
        },
        ...
        initialize: function(){
            ...
            autocomplete_map = new google.maps.Map(document.getElementById(map_window_id), myOptions);
			
            marker = new google.maps.Marker({
                map: autocomplete_map,
                draggable: false
            }); 
            ...
            google.maps.event.addListener(autocomplete_map, 'click', function(event){
